...
void pc_command_processing(void)
{
    if (front != rear)
    {
        ...
        if (strncmp(cmd, "setrtc", 6) == 0 && strlen(cmd) >= 18)
        {
           ...
           // temp[0] = cmd[6]; temp[1] = cmd[7]; temp[2] = 0;
			strncpy(temp, cmd+6, 2);
            ds_time.year = (uint8_t)atoi(temp);   // itoa

            // temp[0] = cmd[8]; temp[1] = cmd[9]; temp[2] = 0;
			strncpy(temp, cmd+8, 2);
            ds_time.month = (uint8_t)atoi(temp);

            temp[0] = cmd[10]; temp[1] = cmd[11]; temp[2] = 0;
            ds_time.date = (uint8_t)atoi(temp);

            temp[0] = cmd[12]; temp[1] = cmd[13]; temp[2] = 0;
            ds_time.hours = (uint8_t)atoi(temp);

            temp[0] = cmd[14]; temp[1] = cmd[15]; temp[2] = 0;
            ds_time.minutes = (uint8_t)atoi(temp);

            temp[0] = cmd[16]; temp[1] = cmd[17]; temp[2] = 0;
